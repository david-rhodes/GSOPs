...
namespace GaussianSplatting.Editor
{
    ...
    public class GaussianSplatAssetCreator : EditorWindow
    {
        ...
        [SerializeField] string m_InputDirectory;
        ...
        void OnGUI()
        {
            ...
            m_InputDirectory = m_FilePicker.PathFieldGUI(rect, new GUIContent("Input Directory"), m_InputDirectory, null, "PointCloudDirectory");
            ...
            {
                if (!string.IsNullOrWhiteSpace(m_InputDirectory) && Directory.Exists(m_InputDirectory))
                {
                    var plyFiles = Directory.GetFiles(m_InputDirectory, "*.ply");
        ...
        unsafe void CreateAsset(string path)
        {
            m_ErrorMessage = null;
            ...
            using NativeArray<InputSplatData> inputSplats = LoadPLYSplatFile(path);
            ...
            string baseName = Path.GetFileNameWithoutExtension(FilePickerControl.PathToDisplayString(path));
